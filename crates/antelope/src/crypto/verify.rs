...
pub fn verify_message(signature: &Signature, message_bytes: &Vec<u8>, pub_key: &Vec<u8>) -> bool {
    ...
    match key_type {
        KeyType::K1 => {
            let public_key_point = k256::PublicKey::from_sec1_bytes(pub_key.as_slice())
                .unwrap()
                .to_encoded_point(false);
            let verifying_key =
                k256::ecdsa::VerifyingKey::from_encoded_point(&public_key_point).unwrap();
        ...
        KeyType::R1 => {
            let public_key_point = p256::PublicKey::from_sec1_bytes(pub_key.as_slice())
                .unwrap()
                .to_encoded_point(false);
            let verifying_key =
                p256::ecdsa::VerifyingKey::from_encoded_point(&public_key_point).unwrap();
